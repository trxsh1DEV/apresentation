...
const useProductivityData = (id: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      try {
        const result = await requestWithToken.get(`/productivity-users/${id}?length=10`);
        return result.data;
      } catch (error: any) {
...
const ActivityMonitor: FC<ProductivityProps> = ({ id, hoursTotal }) => {
  ...
  if (isError) {
    return (
      <div className="w-full h-96 flex items-center justify-center text-4xl">
        Erro ao carregar dados (Talvez não existam dados o suficiente)
      </div>
    );
  }
  ...
  const productivityData = data?.productivity[currentMonth]?.windowTracker || null;
  if (!productivityData) {
    return (
      <div className="w-full h-96 flex items-center justify-center text-4xl">
        Erro ao carregar dados (Talvez não existam dados o suficiente)
  ...
  const sortedData = [productivityData].sort((a, b) => b.quantity - a.quantity);
          ...
          <div className="h-[600px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart layout="vertical" data={chartData} margin={{ top: 20, right: 30, left: 40, bottom: 5 }}>
                <XAxis type="number" />
                <YAxis dataKey="activity" type="category" width={250} tick={{ fontSize: 18, fill: "#888" }} />
